...
大概思路：zCore 中与硬件交互的模块是 HAL 硬件抽象层，将 zCore 移植到 RISCV 上需要将这部分进行修改使其兼容 RISCV 架构。目前的大概思路是先复现 HAL 中的一小部分 API 和这些 API 支持下的上层建筑，然后再一步步增加对 HAL 中的 API 的 RISCV 下的重构。最终达到能在 RISCV 架构下的 QEMU 中运行基本完整的 zCore 。  
预测会遇到的困难：  
+ 需要时间学习 zCore
+ 对于一些基础的系统知识，需要补缺
+ 需要熟悉 QEMU
